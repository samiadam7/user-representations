...
def get_main_artist_column(df: pd.DataFrame, column: str) -> pd.DataFrame:
...
def main():
    ...
    cat_transformer = transformations.make_transformer(
    ...
    preprocessor = transformations.make_preprocessor(
        [
            (
                num_transformer,
                make_column_selector(dtype_include=cast(Any, np.number)),
            ),
            (
                cat_transformer,
                make_column_selector(dtype_exclude=cast(Any, np.number)),
            ),
        ]
    )
    cleaning_steps = [
        partial(
            transformations.drop_na_and_duplicates,
            subset=["name", "main_artist", "year"],
        ),
        transformations.shuffle_data,
    ]

    config = PreprocessorConfig(
        cleaning_steps=cast(List[CleaningStep], cleaning_steps),
    ...
    X = cast(pd.DataFrame, track_preprocessor.fit_transform(feature_df))
